# Write code to watch each splitting events
# Go a few frames before and after each splitting event and watch
...
splitting_df = pd.DataFrame(columns = cols)
...
for i in range(97, 50, -1) :
    ...
    if (df_step['Event'] == 'Splitting').any():

        rows_to_add = df_step.loc[df_step['Event'].isin(['Splitting'])]

        splitting_df = pd.concat([splitting_df, rows_to_add])

        
print('Yay')

for j in range(splitting_df.shape[0]):

    split_event = splitting_df.iloc[j]

    time_of_event = split_event['Timestep']
    clusters_in_event = literal_eval(split_event['Clusters in event'])
