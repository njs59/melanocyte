...
def threshold_arr_supervised(tf_array, threshold, dim):
    ...
    Outputs:
        tf_array_bool: 3D boolean array after thresholding has occurred
    '''
    tf_ad = tf_array
    ...
    if dim == 3:
        # reshaping the array from 3D matrix to 2D matrix.
...
def remove_fragments(area, num_clus, min_clus_size):
    ...
    # Get a list of indexes for slice of array that are not large enough to be considered a cluster
